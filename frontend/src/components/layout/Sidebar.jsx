...
const Sidebar = () => {
  ...
  const handleFollowFromSidebar = async (targetUser) => {
    // Optimistic update (add to following list)
    queryClient.setQueryData(["following", currentUser._id], (old = []) => {
      // avoid duplicate
      const alreadyExists = old.some((u) => u._id?.toString() === targetUser._id?.toString());

      if (alreadyExists) return old;

      return [targetUser, ...old]; // add on top
    });

    try {
      await follow(targetUser._id);
